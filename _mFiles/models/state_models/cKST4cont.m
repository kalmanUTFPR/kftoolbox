% Class for the Kinematic Single Track Model for continuous-time with 4 states:
...
classdef cKST4cont < handle
  ...
  methods(Access = public)
    function obj = cKST4cont(arg_p, arg_dt)
      obj.dt = arg_dt;  
      obj.p = arg_p;
      obj.lwb = obj.p.a + obj.p.b;  
      obj.stMdl = @(x,u)[ x(4) * cos(x(3)); ...
                          x(4) * sin(x(3));...
                          x(4) * tan(u(1)) / obj.lwb;...
                          u(2); ];                                 
    end
    ...
    function x_kp1 = solver(obj,arg_x, arg_u)        
      % Method Runge-Kutta 4 to solve the ODE
      for j = 1:size(arg_x,2)
        x_kp1(:,j) = rk4(obj.stMdl, obj.dt, arg_x(:,j), arg_u);
      end
    end
