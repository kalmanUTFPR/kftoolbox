function run_ukf_kst(arg_axes_state1, arg_axes_state2, arg_axes_state3, arg_axes_state4, arg_axes_traj, r_factor, q_factor)
% Set the parameters
...
% Models
ohMdl = cKST4cont(p, dt); % state model
ohMeas = cKSTMeasMdl;           % measurement model
...
%% RUN FILTER
Q = 10^-4*(diag([0.0022;0.0044;0.0055;0.0089]));
oukf = cUKF(ohMdl,ohMeas,method,kappa,alpha,beta,Q,R);
oukf.initialize(loc_x0, loc_u0, loc_P0);

for i = 1 : datalen 
    u = uinp(:,i);    
    oukf.prediction(u);    
    stateEstimate = oukf.getStateEstimate();
